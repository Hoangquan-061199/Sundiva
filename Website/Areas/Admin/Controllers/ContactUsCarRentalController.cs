...
namespace Website.Areas.Admin.Controllers
{
    public class ContactUsCarRentalController : BaseController
    {
        ...
        public IActionResult Index()
        {
            ...
            Module module = _moduleAdminDa.GetTag("ContactUsCarRental");
        ...
        public ActionResult ProcessExportFile()
        {
            ...
            string fileName = string.Format("thue_xe_{0}.xlsx", DateTime.Now.ToString("yyyy-MM-dd-HH-mm-ss"));
        ...
        public virtual void ExportReportToExcel(string filePath, List<ContactUsAdmin> report)
        {
            ...
            {
                ExcelWorksheet worksheet = xlPackage.Workbook.Worksheets.Add("ThueXe");
                ...
                properties[4] = "Dịch vụ xe";
                properties[5] = "Ngày đi";
                properties[6] = "Ngày về";
                properties[7] = "Điểm đến";
                properties[8] = "Điểm đi";
                properties[9] = "Nội dung";
                properties[10] = "Ngày gửi";
                ...
                foreach (ContactUsAdmin item in report)
                {
                    ...
                    col++;
                    if (item.Service != null)// Dịch vụ xe
                        worksheet.Cells[row, col].Value = item.Service;
                    col++;
                    if (item.StartDate.HasValue)// Ngày đi
                        worksheet.Cells[row, col].Value = item.StartDate.Value.ToString("dd/MM/yyyy");
                    col++;
                    if (item.EndDate.HasValue)// Ngày về
                        worksheet.Cells[row, col].Value = item.EndDate.Value.ToString("dd/MM/yyyy");
                    col++;
                    if (item.Destination != null)// Điểm đến
                        worksheet.Cells[row, col].Value = item.Destination;
                    col++;
                    if (item.PointGo != null)// Điểm đi
                        worksheet.Cells[row, col].Value = item.PointGo;
                    col++;
                ...
                string nameexcel = "Danh sách thuê xe " + DateTime.Now.ToString("yyyy-MM-ddTHH:mm:ss.fff");
                ...
                xlPackage.Workbook.Properties.Company = "Thuê xe";
                xlPackage.Save();
            }
        }
    }
